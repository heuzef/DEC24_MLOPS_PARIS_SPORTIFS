...
def parivison_predict(inputs):
    ...
    try:
        ...
        return response_data.get("prediction") # Utiliser .get() est plus sûr car il retourne None si la clé n'existe pas, évitant une KeyError
...
def parivison_get_champion_infos():
    ...
    try:
        ...
        response.raise_for_status()  # Lève une exception pour les codes d'erreur HTTP (4xx ou 5xx)
        response_data = response.json() # Extraire les données JSON de la réponse        
        return response_data.get("champion") # Utiliser .get() est plus sûr car il retourne None si la clé n'existe pas, évitant une KeyError
